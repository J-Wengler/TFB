...
#include <string>
static void decompressOneLine(const char* fname)
{
    
    FILE* const fin  = fopen_orDie(fname, "rb");
    //size_t const buffInSize = ZSTD_DStreamInSize();
    
    size_t const buffInSize = 10001; //Get from .ll file?
    void*  const buffIn  = malloc_orDie(buffInSize);
    FILE* const fout = stdout;
    //size_t const buffOutSize = ZSTD_DStreamOutSize();  /* Guarantee to successfully flush at least one complete compressed block in all circumstances. */
    size_t const buffOutSize = 10001; //Get from .ll file?
   ...
   // int isEmpty = 1;
    ...
    //isEmpty = 0;
    size_t pos = 2627; //Get from .rowstart file (the col you want to read)
